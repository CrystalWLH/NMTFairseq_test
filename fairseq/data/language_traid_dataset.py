...
def collate_traid(
    ...
    if prev_output_tokens_seg is not None:
        #batch['segmentation_input']['prev_output_tokens_seg'] = prev_output_tokens_seg
        ...
        Returns:
            dict: a mini-batch with the following keys:
                ...
                - `net_input` (dict): the input to the Model, containing keys:
                  ...
                  - `segmentation_lengths` (LongTensor): 1D Tensor of the unpadded
                    lengths of each source word sentence of shape `(bsz)`
                  - `prev_output_tokens_seg` (LongTensor): a padded 2D Tensor of
                    tokens in the segmentation sentence, shifted right by one
                    position for teacher forcing, of shape `(bsz, tgt_len)`.
                    This key will not be present if *input_feeding* is
                    ``False``.  Padding will appear on the left if
                    *left_pad_target* is ``True``. 
                - `segmentation_tokens` (LongTensor): a padded 2D Tensor of tokens in the
