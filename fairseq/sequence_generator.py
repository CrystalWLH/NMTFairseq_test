...
#added by lihui
class SequenceGeneratorCTC(object):
    def __init__(
        self,
        tgt_dict,
        beam_size=1,
        ...
        """Generates translations of a given source sentence based on SegCTCNMT model.
    ...
    def _generate(
        ...
        **kwargs
        ...
        encoder_input = {
            k: v for k, v in sample['net_input'].items()
            if k != 'prev_output_tokens' and k != 'segmentation_lengths' and k != 'prev_output_tokens_seg'
        }
...
class EnsembleCTCModel(torch.nn.Module):
    """A wrapper around an ensemble of models."""
    ...
    def __init__(self, models):
        ...
        self.models = torch.nn.ModuleList(models)
        self.incremental_states = None
        if all(isinstance(m.decoder_nmt, FairseqIncrementalDecoder) for m in models):
            self.incremental_states = {m: {} for m in models}

    def has_encoder(self):
        return hasattr(self.models[0], 'encoder_nmt')
    ...
    def _decode_one(
        ...
        if attn is not None:
            attn = attn[:, -1, :]
        probs = model.get_normalized_probs(decoder_out[0], log_probs=log_probs)
        probs = probs[:, -1, :]
        return probs, attn
    ...
    def reorder_encoder_out(self, encoder_outs, new_order):
        if not self.has_encoder():
            return
        return [
            model.encoder_shared.reorder_encoder_out(encoder_out, new_order)
            for model, encoder_out in zip(self.models, encoder_outs)
        ]
    ...
    def reorder_incremental_state(self, new_order):
        if self.incremental_states is None:
            return
        for model in self.models:
            model.decoder_nmt.reorder_incremental_state(self.incremental_states[model], new_order)
