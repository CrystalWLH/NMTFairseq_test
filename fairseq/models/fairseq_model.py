...
'''
    Segmentation + NMT based on Transformer Model.
    Add by Lihui Wang (2019-11-29)
'''
class FairseqEncoderDecoderDoubleModel(BaseFairseqModel):
    """Base class for segmentation + NMT encoder-decoder models.

    Args:
        encoder_seg (FairseqEncoder): the encoder of segmentation
        decoder_seg (FairseqDecoder): the decoder of segmentation
        encoder_nmt (FairseqEncoder): the encoder of NMT
    ...
    def forward(self, src_tokens, src_lengths, segmentation_lengths, prev_output_tokens, prev_output_tokens_seg,  **kwargs):
        ...
        the decoder to produce the next outputs::
            ...
            encoder_out_seg = self.encoder_seg(src_tokens, src_lengths)
            decoder_out_seg = self.decoder_seg(prev_output_tokens_seg, encoder_out_seg)
            encoder_out_nmt = self.encoder_nmt(encoder_out_seg, seg_lengths)
            decoder_out_nmt = self.decoder_nmt(prev_output_tokens, encoder_out_nmt)
        ...
        Args:
            src_tokens (LongTensor): tokens in the source language of shape
                `(batch, src_len)`
            src_lengths (LongTensor): source sentence lengths of shape `(batch)`
            seg_lengths (LongTensor): source word sentence lengths of shape `(batch)`
            prev_output_tokens (LongTensor): previous decoder outputs of shape in NMT
                `(batch, tgt_len)`, for teacher forcing
        ...
        Returns:
            double tuple:
                - the decoder's (ctc / ctc + nmt) output of shape `(batch, tgt_len, vocab)`
                - a dictionary with any model-specific outputs
        """
    ...
    def output_layer(self, features, **kwargs):
        """Project features to the default output size (typically vocabulary size)."""
        return self.decoder_nmt.output_layer(features, **kwargs)
    ...
    def max_decoder_positions_seg(self):
        """Maximum length supported by the decoder."""
