...
#CUDA_VISIBLE_DEVICES=0 python generate.py mydata/data-bin/nmt_seg_bin_wordWord --source-lang seg --target-lang zh --seg-lang seg --task seg_translation --log-format simple --log-interval 10 --path checkpoints/exp4_wordWord/checkpoint340.pt --batch-size 1 --beam 5 --remove-bpe --results-path temp --lenpen 0.6 --max-len-a 1 --max-len-b 50
...
CUDA_VISIBLE_DEVICES=0 python generate.py mydata/data-bin/nmt_seg_ZhEn_bin_wordWord --source-lang seg --target-lang en --seg-lang seg --task seg_translation --log-format simple --log-interval 10 --path checkpoints/exp4_zhen_wordWord/checkpoint_last.pt --batch-size 1 --beam 5 --remove-bpe --results-path temp --lenpen 0.6 --max-len-a 1 --max-len-b 50
